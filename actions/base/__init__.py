...
def item_to_dict(item, include_body=False, folder_name=None):
    result = {
        ...
        'datetime_sent': item.sent if item.sent else None,
        'datetime_received': item.received if item.received else None,
        'datetime_created': item.created if item.datetime_created else None,
